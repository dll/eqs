  ...
  <div class="settings-page">
    <el-card style="margin-bottom: 20px">
      <template #header>
        <div style="display: flex; align-items: center; justify-content: space-between;">
          ...
          <el-select
            ...
            :placeholder="$t('settings.categoryAll')"
            clearable
            style="width: 160px"
          >
            <el-option
              :label="$t('settings.categoryAll')"
              value=""
            />
            <el-option
              :label="$t('settings.categoryTheme')"
              value="theme"
            />
            <el-option
              :label="$t('settings.categoryI18n')"
              value="i18n"
            />
            <el-option
              :label="$t('settings.categoryMulti')"
              value="multiplatform"
            />
            <el-option
              :label="$t('settings.categoryVersion')"
              value="version"
            />
            <el-option
              :label="$t('settings.categoryDemo')"
              value="demo"
            />
            <el-option
              :label="$t('settings.categorySystem')"
              value="system"
            />
          </el-select>
        </div>
      ...
      >
        <el-table-column
          prop="config_key"
          :label="$t('settings.configKey')"
          width="240"
        />
        <el-table-column
          prop="config_value"
          :label="$t('settings.configValue')"
        />
        <el-table-column
          prop="value_type"
          :label="$t('settings.valueType')"
          width="80"
        />
        <el-table-column
          prop="description"
    ...
    <el-card style="margin-bottom: 20px">
      ...
      <el-form inline>
        <el-form-item :label="$t('settings.demoMode')">
          ...
          >
            <el-option
              :label="$t('settings.demoModeDemo')"
              value="demo"
            />
            <el-option
              :label="$t('settings.demoModeTest')"
              value="test"
            />
            <el-option
              :label="$t('settings.demoModeTraining')"
              value="training"
            />
          </el-select>
        </el-form-item>
        <el-form-item>
          ...
          <el-button
            ...
            @click="cleanDemo"
          >
            {{ $t('settings.demoClean') }}
          </el-button>
          <el-button @click="loadDemoStatus">
            {{ $t('settings.demoRefresh') }}
          </el-button>
        </el-form-item>
      </el-form>
      <el-descriptions
      ...
      >
        <el-descriptions-item :label="$t('settings.demoStatus')">
          {{ demoStatus.demo_mode ? $t('settings.demoEnabled') : $t('settings.demoDisabled') }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('settings.demoUsers')">
          {{ demoStatus.user_count || 0 }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('settings.demoProjects')">
          {{ demoStatus.project_count || 0 }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('settings.demoOrders')">
          {{ demoStatus.order_count || 0 }}
        </el-descriptions-item>
        <el-descriptions-item :label="$t('settings.demoDisputes')">
          {{ demoStatus.dispute_count || 0 }}
        </el-descriptions-item>
      </el-descriptions>
      <el-alert
        type="info"
        :closable="false"
        :title="$t('settings.demoTip')"
        style="margin-top: 12px;"
      />
    </el-card>
    ...
    <el-card style="margin-bottom: 20px">
      ...
      >
        <el-table-column
          prop="version"
          :label="$t('settings.versionNo')"
          width="100"
        />
        <el-table-column
          prop="build"
          :label="$t('settings.buildNo')"
          width="90"
        />
        <el-table-column
          prop="platform"
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column
          prop="count"
          :label="$t('settings.monitorCount')"
          width="100"
        />
        <el-table-column
          prop="error_count"
          :label="$t('settings.monitorErrors')"
          width="100"
        />
        <el-table-column
          prop="avg_ms"
          :label="$t('settings.monitorAvg')"
          width="120"
        >
          <template #default="{ row }">
            {{ row.avg_ms?.toFixed(1) || '0' }}
          </template>
        </el-table-column>
        <el-table-column
          prop="p95_ms"
          :label="$t('settings.monitorP95')"
          width="120"
        >
          <template #default="{ row }">
            {{ row.p95_ms?.toFixed(1) || '0' }}
          </template>
        </el-table-column>
        <el-table-column
          prop="error_rate"
          :label="$t('settings.monitorErrorRate')"
          width="110"
        >
          <template #default="{ row }">
            {{ row.error_rate?.toFixed(1) || '0' }}%
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    ...
    >
      <el-form
        ...
        label-width="100px"
      >
        <el-form-item :label="$t('settings.configKey')">
          <el-input v-model="configForm.config_key" />
        </el-form-item>
        <el-form-item :label="$t('settings.configValue')">
        ...
        <el-form-item :label="$t('settings.valueType')">
          ...
          >
            <el-option
              :label="$t('settings.typeString')"
              value="string"
            />
            <el-option
              :label="$t('settings.typeInt')"
              value="int"
            />
            <el-option
              :label="$t('settings.typeBool')"
              value="bool"
            />
            <el-option
              :label="$t('settings.typeJson')"
              value="json"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item :label="$t('settings.isPublic')">
    ...
    <el-dialog
      ...
      :title="$t('settings.publishVersion')"
      width="500px"
    >
      <el-form
        :model="versionForm"
        label-width="100px"
      >
        <el-form-item :label="$t('settings.versionNo')">
          <el-input
            v-model="versionForm.version"
            :placeholder="$t('settings.placeholderVersion')"
          />
        </el-form-item>
        <el-form-item :label="$t('settings.buildNo')">
          <el-input-number v-model="versionForm.build" />
        </el-form-item>
        <el-form-item :label="$t('settings.platform')">
          ...
          >
            <el-option
              :label="$t('settings.platformAll')"
              value="all"
            />
            <el-option
              :label="$t('settings.platformH5')"
              value="h5"
            />
            <el-option
              :label="$t('settings.platformMp')"
              value="mp-weixin"
            />
            <el-option
              :label="$t('settings.platformApp')"
              value="app"
            />
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('settings.updateUrl')">
          <el-input v-model="versionForm.update_url" />
        </el-form-item>
        <el-form-item :label="$t('settings.releaseNotes')">
          <el-input
            v-model="versionForm.release_notes"
            type="textarea"
          />
        </el-form-item>
        <el-form-item :label="$t('settings.forceUpdate')">
          <el-switch v-model="versionForm.mandatory" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="versionDialog = false">
          {{ $t('common.cancel') }}
        </el-button>
        <el-button
          type="primary"
